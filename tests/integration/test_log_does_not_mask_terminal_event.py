...
def test_log_event_after_done_does_not_mask_terminal_transition(
    compose_stack, run_naiw_tasks, seed_project,
):
    ...
    # try/finally so an assertion failure does not leak the running
    # container into the shared session data_root. Without this, a
    # regression here would also corrupt downstream tests in the same
    # CI run.
    try:
        # Emit BOTH events back-to-back inside the same shell so they
        # land in one read window — `done` first, then a status-neutral
        # `log`. The log MUST be transparent to the reconciler.
        subprocess.run(
            [
                "docker", "exec", container,
                "sh", "-c",
                'naiw-signal done --summary "task ok" && '
                'naiw-signal log "wrapping up"',
            ],
            env=env, check=True, capture_output=True,
        )

        # Drive reconcile. The lazy-event tailer applies the events on the
        # next `naiw-tasks list`. With the bug, status stays "running"
        # because "log" was picked as the pending kind and compute_status
        # had no log branch. With the fix, "done" wins despite "log" being
        # newer, and the row transitions to "completed".
        transitioned = False
        last_status = None
        for _ in range(15):
        ...
        assert transitioned, (
            f"task stuck at status={last_status!r} — log event masked "
            f"the terminal kind. See "
            f"reconcile.select_latest_transition_kind."
        )

        # Belt-and-braces: events_offset is past BOTH events (the log
        # event was consumed transparently, not deferred).
        task_json = _read_task_json(task_dir)
        assert task_json["events_offset"] > 0, task_json
    finally:
        # Best-effort cleanup. Use finish + clean defensively — finish
        # may already have been driven by the auto_finish-equivalent
        # path, in which case clean is enough; if status is still
        # running, finish stops the container first.
        run_naiw_tasks("finish", task_id)
        run_naiw_tasks("clean", "--older-than", "0s", "--yes")
